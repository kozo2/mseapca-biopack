...
#' @param DET A character vector of all detected metabolite IDs (background). Required for all methods except \code{ora_full}.
#' @param M A named list of metabolite sets, each containing a character vector of metabolite IDs.
#' @param option Method to use for estimating the p-value range. One of \code{"ora_full"}, \code{"bino_naive"}, \code{"bino_weighted"}, or \code{"bino_shrink"}.
#' @param probs Numeric vector of quantile probabilities for binomial simulation (e.g., \code{c(0.025, 0.975)}). Ignored for \code{ora_full}.
...
#'
#' @return A list containing a matrix with p-value range results for each metabolite set. Columns include lower, median, and upper p-values.
#'
#' @details
